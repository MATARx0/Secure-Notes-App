...
| T-07 | Tampering | A note title containing markup is rendered as HTML, executing script for whoever views the list (stored XSS). | Session or note theft in the victim's own browser; persists in the database until the note is deleted. | The note list is built with `createElement` and `textContent`, so stored data can never become markup. CSP (`script-src 'self'`, no `unsafe-inline`) remains as the second layer. A static scan in the test suite fails if any frontend script reintroduces `innerHTML`, `insertAdjacentHTML`, `document.write` or `eval`. | `public/js/notes.js`, `tests/security/hardening.test.js` |
